...
public class DiegeticButton : Interactable
{
    ...
    void Update()
    {
        ...
        if (touchingCtrl != null) // If we are touching button cap
        {
            ...
            Vector3 controllerLocalPos = transform.parent.InverseTransformPoint(touchingCtrl.transform.position);
            Vector3 capToController = controllerLocalPos - restLocalPos;


            // --- TODO - Signed distance along press direction (use dot product) 
            float signedDist = Vector3.Dot(capToController, -axisLocal);
            ...
            float depth = Mathf.Max(0f, signedDist - deadZone);
            pressDepth = Mathf.Clamp(depth, 0f, maxPressDepth);
            ...
            if (!armed && Time.time >= nextAllowedTime && pressDepth >= maxPressDepth * pressThreshold)
            {
                ...
                if (haptics)
                {
                    ...
                    touchingCtrl.HapticClick(clickAmplitude, clickDuration);
                }
            }
        ...
        transform.localPosition = restLocalPos - axisLocal * pressDepth;
    }
